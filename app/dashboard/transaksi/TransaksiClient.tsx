...
export default function TransaksiClient({
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Apakah Anda yakin ingin menghapus ${selectedIds.length} transaksi yang dipilih?`)) return;
    setIsDeleting(true);
    setDeleteError("");
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (catFilterRef.current && !catFilterRef.current.contains(event.target as Node)) setCatDropdownOpen(false);
      if (projFilterRef.current && !projFilterRef.current.contains(event.target as Node)) setProjDropdownOpen(false);
      if (itemsPerPageRef.current && !itemsPerPageRef.current.contains(event.target as Node)) setItemsPerPageOpen(false);
    };
  ...
  return (
    <div className="text-gray-600 dark:text-gray-300 w-full h-full">
            ...
            <div className="relative w-full md:w-auto" ref={catFilterRef}>
              ...
              {catDropdownOpen && (
                <div className="absolute z-50 right-0 mt-3 w-56 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden flex flex-col p-1.5 animate-in fade-in zoom-in-95 duration-200">
                  <button
                    type="button"
                    onClick={() => { setCategory(""); setCatDropdownOpen(false); }}
                    className={`text-left px-3 py-2 text-sm font-semibold rounded-md transition-colors ${category === "" ? "bg-[#EA6C00] text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 hover:text-[#EA6C00]"}`}
                  >
                    Semua Kategori
                  </button>
            ...
            <div className="relative w-full md:w-auto" ref={projFilterRef}>
              ...
              >
                <span className="truncate max-w-[150px]">
                  {projectFilter ? projects.find(p => p.id === projectFilter)?.code : "Semua Proyek"}
                </span>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
              ...
              {projDropdownOpen && (
                <div className="absolute z-50 right-0 mt-3 w-64 max-h-60 overflow-y-auto bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl shadow-xl flex flex-col p-1.5 animate-in fade-in zoom-in-95 duration-200">
                  <button
                    type="button"
                    onClick={() => { setProjectFilter(""); setProjDropdownOpen(false); }}
                    className={`text-left px-3 py-2 text-sm font-semibold rounded-md transition-colors ${projectFilter === "" ? "bg-[#EA6C00] text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 hover:text-[#EA6C00]"}`}
                  >
                    Semua Proyek
                  </button>
                  {projects.map((p) => (
                    <button
                      type="button"
                      key={p.id}
                      onClick={() => { setProjectFilter(p.id); setProjDropdownOpen(false); }}
                      className={`text-left px-3 py-2 text-sm font-semibold rounded-md transition-colors ${projectFilter === p.id ? "bg-[#EA6C00] text-white" : "text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 hover:text-[#EA6C00]"}`}
                    >
                      {p.code} — {p.name}
                    </button>
                  ))}
        ...
        <div className="bg-white dark:bg-slate-800 border-[0.5px] border-[#E5E7EB] dark:border-slate-700/50 rounded-[14px] shadow-sm overflow-hidden">
          {filteredTransactions.length === 0 ? (
            <div className="p-16 text-center">
              ...
              <p className="font-bold text-gray-900 dark:text-white">Belum ada transaksi</p>
              <p className="text-sm text-gray-400 mt-1 italic">Klik "Tambah Transaksi" untuk memulai pencatatan.</p>
            </div>
          ) : (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 whitespace-nowrap text-center text-gray-400">
                        <div className="flex items-center justify-center gap-3">
                          ...
                          <button
                            onClick={() => setDeleteModal({ show: true, type: "single", id: trx.id })}
                            disabled={isDeleting}
          ...
          {filteredTransactions.length > 0 && (
            <div className="px-5 py-4 bg-white dark:bg-slate-800 border-t border-[#F3F4F6] dark:border-slate-700/50 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="text-sm text-gray-500 dark:text-gray-400 order-2 md:order-1">
                Total Transaksi: <span className="font-bold text-gray-900 dark:text-white">{filteredTransactions.length}</span>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white dark:bg-slate-800 rounded-[28px] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200 text-center">
              <div className="w-16 h-16 bg-red-50 dark:bg-red-900/20 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-8 h-8 text-[#EF4444]">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                </svg>
              </div>
              
              <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                {deleteModal.type === "bulk" ? `Hapus ${selectedIds.length} Transaksi?` : "Hapus Transaksi?"}
              </h3>
      ...
      {/* Add Modal */}
      {showAddModal && <AddTransaksiModal projects={projects} units={units} customers={customers} />}
