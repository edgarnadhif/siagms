...
function CancelPurchaseModal({
  ...
  const [tanggalBatal, setTanggalBatal] = useState(
        ...
        <form onSubmit={handleSubmit} className="flex flex-col overflow-y-auto">
          <div className="p-6 space-y-5">
            ...
            <div className="bg-slate-100 dark:bg-slate-800/80 rounded-xl p-4 border border-slate-100 dark:border-slate-700 space-y-2.5">
              <h4 className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3">
                Ringkasan Pembatalan
              </h4>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500 font-medium">Unit</span>
                <span className="font-bold text-slate-800 dark:text-white">
                  {unit.unitCode} — Blok {unit.blockName} No.{unit.unitNumber}
                </span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm border-t border-slate-100 dark:border-slate-700 pt-2.5 mt-2">
                ...
                <span
                  className={`font-black text-base ${totalBF > 0 ? "text-red-600" : "text-slate-400"}`}
                >
                  Rp {formattedBF}
                </span>
              </div>
              {totalBF > 0 && (
                <p className="text-[11px] text-amber-600 dark:text-amber-400 font-semibold bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/30 rounded-lg px-3 py-2">
                  ⚠️ Booking Fee sebesar Rp {formattedBF} akan dicatat sebagai{" "}
                  <strong>Pendapatan Lain-lain (akun 4200)</strong> dan tidak
            ...
            <div>
              ...
              />
              <p
                className={`text-[11px] mt-1 font-medium ${alasan.length < 10 && alasan.length > 0 ? "text-red-500" : "text-gray-400"}`}
              >
            ...
            <label className="flex items-start gap-3 cursor-pointer group">
              ...
              <span className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                ...
                <strong className="text-red-600">Rp {formattedBF}</strong> tidak
                akan dikembalikan ke pembeli dan akan dicatat sebagai pendapatan
                lain-lain.
              </span>
            </label>
...
function AkadProcessModal({ unit, onClose, onSuccess }: AkadProcessModalProps) {
  ...
  const [nilaiKPR, setNilaiKPR] = useState(
    String(Number(unit.customer?.kprAmount || 0) || ""),
  );
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (
      ...
      setError(
        "Tanggal akad, nama bank, nomor akad, dan nilai KPR wajib diisi",
      );
      return;
    }
          ...
          <div className="w-11 h-11 rounded-xl bg-orange-100 dark:bg-orange-900/40 flex items-center justify-center flex-shrink-0">
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth={2}
                d="M8.25 6.75h7.5m-7.5 4.5h7.5m-7.5 4.5h4.5M3.75 5.25A2.25 2.25 0 016 3h12a2.25 2.25 0 012.25 2.25v13.5A2.25 2.25 0 0118 21H6a2.25 2.25 0 01-2.25-2.25V5.25z"
              />
            </svg>
          </div>
          <div className="flex-1">
            ...
            <p className="text-sm text-orange-700/80 dark:text-orange-400/80 mt-0.5">
              Simpan data akad sebelum pencairan KPR diproses.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col">
          <div className="p-6 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full h-11 px-4 rounded-[10px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm"
                  required
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={nomorAkad}
                  onChange={(e) => setNomorAkad(e.target.value)}
                  className="w-full h-11 px-4 rounded-[10px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm"
                  required
                />
              </div>
              <div>
                ...
                <input
                  ...
                  min="1"
                  value={nilaiKPR}
                  onChange={(e) => setNilaiKPR(e.target.value)}
                  className="w-full h-11 px-4 rounded-[10px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm"
                  required
                />
              </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full p-4 rounded-[10px] border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm resize-none"
                placeholder="Catatan tambahan akad (opsional)"
              />
            </div>

            {error && (
              <div className="text-red-500 text-xs font-bold p-3 bg-red-50 dark:bg-red-950/20 rounded-xl border border-red-100 dark:border-red-900/30">
                {error}
              </div>
            )}
          </div>
          ...
          <div className="px-6 pb-6 flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 h-11 text-sm font-bold text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-[10px] transition-all"
            >
              Batal
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-[2] h-11 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold rounded-[10px] shadow-lg shadow-orange-500/20 transition-all"
            >
...
export default function UnitDetailModal({
  ...
  useEffect(() => {
    if (stState?.success) {
      ...
      setShowSTForm(false);
      showToast(
        stState.message || "Serah terima berhasil diproses.",
        "success",
      );
    }
  ...
  const totalPaid = unit.transactions.reduce(
  ...
  const stBreakdown = unit.transactions.reduce(
    (
      ...
      if (transaction.category === "PELUNASAN_CASH")
        acc.pelunasanCash += amount;
      acc.total += amount;
      return acc;
    },
    {
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        <div className="bg-white dark:bg-slate-800 rounded-[16px] shadow-2xl w-full max-w-4xl overflow-hidden border border-white/20 dark:border-slate-700 max-h-[90vh] flex flex-col">
          {/* Header */}
          <div className="flex justify-between items-center p-5 border-b border-[#F3F4F6] dark:border-slate-700 bg-white dark:bg-slate-800">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 bg-orange-50 dark:bg-orange-500/10 rounded-xl flex items-center justify-center text-[#EA6C00] border border-orange-100 dark:border-orange-900/30">
                <img
                  src="/perumahan.svg"
                  alt="Perumahan"
                  className="w-5 h-5 filter grayscale brightness-75 opacity-80"
                />
              </div>
              <div>
                ...
                <p className="text-sm text-slate-500 font-medium">
                  Blok {unit.blockName} No.{unit.unitNumber} •{" "}
                  {unit.project?.name}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={onClose}
                className="p-2 text-slate-400 hover:text-[#EA6C00] transition-colors rounded-full hover:bg-gray-50 dark:hover:bg-slate-700"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 subtle-scrollbar">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {/* Payment Progress */}
              <div className="md:col-span-2 bg-slate-50 dark:bg-slate-900/30 rounded-2xl p-5 border border-[#E5E7EB] dark:border-slate-700">
              ...
              {/* Customer Brief */}
              <div className="bg-white dark:bg-slate-900/30 text-slate-800 dark:text-slate-100 rounded-2xl p-5 border border-[#E5E7EB] dark:border-slate-700 relative overflow-hidden">
                <div className="relative z-10">
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">
                    Pelanggan
                  </p>
                  <h4 className="text-lg font-bold mb-4 text-slate-900 dark:text-white">
                    {unit.customer?.name || "Belum ada pembeli"}
                  </h4>
                  <div className="space-y-2 text-sm text-slate-600 dark:text-slate-300">
                    <div className="flex justify-between border-b border-slate-100 dark:border-slate-700 pb-2">
                      <span>Metode</span>
                      <span className="font-bold">
                        {unit.customer?.paymentMethod || "—"}
                      </span>
                    </div>
                ...
                <svg
                  className="absolute -bottom-4 -right-4 w-24 h-24 text-orange-500/5"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div className="relative pt-8 pb-4">
                ...
                <div className="flex justify-between relative z-10">
                  {timelineSteps.map((step, idx) => {
                    const isActive =
                      getStatusStep(unit.status) >= getStatusStep(step.id);
                    return (
                      <div
                        key={step.id}
                      ...
                      >
                        ...
                        <div className="mt-3 text-center">
                          <p
                            className={`text-xs font-bold transition-colors ${isActive ? "text-slate-800 dark:text-white" : "text-slate-400"}`}
                          >
                            {step.label}
                          </p>
                          ...
                          {step.id === "AKAD" && canInputPencairan && (
                            <button
                              onClick={() =>
                                router.push(
                                  `/dashboard/transaksi?add=true&category=PENCAIRAN_KPR&unitId=${unit.id}&projectId=${unit.projectId}&customerId=${unit.customerId}`,
                                )
                              }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-slate-700">
                    {unit.transactions.length === 0 ? (
                      ...
                      unit.transactions.map((t: any) => (
                        <div
                          key={t.id}
                          className="p-4 flex justify-between items-center hover:bg-white dark:hover:bg-slate-800 transition-colors"
                        >
                          <div>
                          ...
                          <div className="text-right">
                            ...
                            <span className="text-[9px] font-bold px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded uppercase tracking-tighter">
                              LUNAS
                            </span>
                          </div>
                        </div>
              ...
              <div className="space-y-4">
                ...
                {unit.status === "SERAH_TERIMA" ? (
                  <div className="bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 p-6 rounded-2xl flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-emerald-600/80 mt-1">
                      Pendapatan telah diakui dalam sistem akuntansi secara
                      otomatis.
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/30 border border-dashed border-slate-300 dark:border-slate-700 p-8 rounded-2xl flex flex-col items-center text-center">
                    <p className="text-slate-400 text-sm font-medium">
                      ...
                      pencairan dan serah terima.
                    </p>
                  </div>
                ) : !isFullyPaid ? (
                  <div className="bg-slate-50 dark:bg-slate-900/30 border border-dashed border-slate-300 dark:border-slate-700 p-8 rounded-2xl flex flex-col items-center text-center">
                    <p className="text-slate-400 text-sm font-medium">
                      Selesaikan pelunasan 100% untuk mengaktifkan fitur Serah
                      Terima.
                    </p>
                    {canInputPencairan && (
                      <button
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/30 border border-dashed border-slate-300 dark:border-slate-700 p-8 rounded-2xl flex flex-col items-center text-center">
                    <p className="text-slate-400 text-sm font-medium">
                      Belum ada Booking Fee, Down Payment, Pencairan KPR, atau
                      Pelunasan Cash yang bisa diakui saat serah terima.
                    </p>
                  </div>
                ) : !showSTForm ? (
                  <button
                    onClick={() => setShowSTForm(true)}
                    className="w-full h-12 bg-[#EA6C00] hover:bg-[#C25500] text-white rounded-xl font-bold text-sm shadow-lg shadow-orange-500/20 transition-all active:scale-[0.98] flex items-center justify-center gap-3"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                  ...
                  >
                    ...
                    <div className="rounded-2xl border border-orange-100 dark:border-orange-900/30 bg-orange-50/70 dark:bg-orange-950/20 p-4 space-y-3">
                      <div>
                        <p className="text-[11px] font-black text-orange-500 uppercase tracking-[0.2em]">
                          Konfirmasi Pengakuan Pendapatan
                        </p>
                        <h4 className="text-base font-black text-slate-800 dark:text-white mt-1">
                          Total pendapatan yang akan diakui:{" "}
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5 ml-1">
                        No. Berita Acara{" "}
                        <span className="text-[#EA6C00]">*</span>
                      </label>
                      <input
                        type="text"
                        name="handoverNo"
                        required
                        placeholder="Contoh: BA-ST/2026/001"
                        className="w-full h-11 px-4 bg-slate-50 dark:bg-slate-900 border border-gray-100 dark:border-slate-700 rounded-[10px] text-sm text-gray-900 dark:text-white focus:ring-4 focus:ring-[#EA6C00]/10 focus:border-[#EA6C00] outline-none transition-all placeholder-gray-400"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1.5 ml-1">
                      ...
                      <input
                        ...
                        className="w-full h-11 px-4 bg-slate-50 dark:bg-slate-900 border border-gray-100 dark:border-slate-700 rounded-[10px] text-sm text-gray-900 dark:text-white focus:ring-4 focus:ring-[#EA6C00]/10 focus:border-[#EA6C00] outline-none transition-all"
                      />
                    </div>
                    {stState?.error && (
                      <div className="text-red-500 text-xs font-bold p-3 bg-red-50 dark:bg-red-950/20 rounded-xl border border-red-100 dark:border-red-900/30">
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <button
                        type="submit"
                        disabled={isStPending || !canProcessST}
                        className="flex-[2] h-11 text-sm font-black text-white bg-[#EA6C00] hover:bg-[#C25500] rounded-[10px] shadow-lg shadow-orange-500/20 transition-all active:scale-95 disabled:opacity-50"
                      >
                        ...
                        <div className="flex justify-between items-start gap-4 mb-3">
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider mt-0.5">
                              {c.customerCode} • Dibatalkan{" "}
                              {new Date(c.tanggalBatal).toLocaleDateString(
                                "id-ID",
                                {
                                  day: "numeric",
                                  month: "long",
